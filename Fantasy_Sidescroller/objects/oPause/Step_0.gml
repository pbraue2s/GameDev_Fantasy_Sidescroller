...
}else if(room == Bibliothek and keyboard_check_pressed(ord("E")) and !pause){
	if(collision_circle(oTeleportBookLevel1.x, oTeleportBookLevel1.y, oTeleportBookLevel1.radius, oCharacter, false, true)){
		instance_create_layer(camera_get_view_x(view_camera[0])-5, camera_get_view_y(view_camera[0])-5, "Menue", oPauseBackground);
		instance_create_layer(camera_get_view_x(view_camera[0])+180, camera_get_view_y(view_camera[0])+44, "MenueBook", oLevel1StartBook);
	}else if(collision_circle(oTeleportBookLevel2.x, oTeleportBookLevel2.y, oTeleportBookLevel2.radius, oCharacter, false, true)){
		instance_create_layer(camera_get_view_x(view_camera[0])-5, camera_get_view_y(view_camera[0])-5, "Menue", oPauseBackground);
		instance_create_layer(camera_get_view_x(view_camera[0])+180, camera_get_view_y(view_camera[0])+44, "MenueBook", oLevel2StartBook);
	}else if(collision_circle(oTeleportBookLevel3.x, oTeleportBookLevel3.y, oTeleportBookLevel3.radius, oCharacter, false, true)){
		instance_create_layer(camera_get_view_x(view_camera[0])-5, camera_get_view_y(view_camera[0])-5, "Menue", oPauseBackground);
